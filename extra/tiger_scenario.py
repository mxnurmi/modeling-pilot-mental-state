...
def test_planner(tiger_problem, planner, nsteps=3, debug_tree=False):
    ...
    for i in range(nsteps):
        ...
        print("xmas tree")
        try:
            print(tiger_problem.agent.tree)
            agent = tiger_problem.agent
            print("our outsomes")
            print(action)
            print(real_observation)
            print(agent.tree[action][real_observation])
        except AttributeError as e:
            print("It happened again man:", e)


        planner.update(tiger_problem.agent, action, real_observation) # HERE
